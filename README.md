...
Ephemeral Mode a feature that clears all local data and deletes the user profile on each restart. This makes some exploits (like RigTools) non-persistent. However, it also reduces storage capacity so it is recommended to use Google Drive as the default download directory (`${google_drive}/`). Refer to the [google documentation](https://support.google.com/chrome/a/answer/3538894).
